 ...
 * $Log: geCgiExecuteSearchBeer.c,v $
...
main ( int argc, char *argv[] )
{
	...
	FILE *reading;	
	char geSearchBeer [NICKNAME_MAX_LENGTH + 1];
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "searchBeerName", geSearchBeer, BEER_NAME_MAX_LENGTH ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
	...
	if ( language == portuguese )
	{
		...
		printf("    Procurar Cervejas: <br>\n" );
		printf ("<form action=\"geChosenSearchBeer.cgi\" method=\"post\"> \n" );
		...
		while ( fgets ( linha, BEER_NAME_MAX_LENGTH + COMMENT_MAX_LENGTH + 8, reading ) != NULL )
		{
			...
			if ( linha[0] == 'b' )
				if ( linha[1] == '|' )
					if ( strstr ( &linha[2] , geSearchBeer ) != NULL )
						printf ( "<input type='radio' name='geBeerName' value='%s'><b>%s</b><br>\n", &linha[2], &linha[2] );
		}		
		printf ("  <input type=\"submit\" name=\"submit\" value=\"Ver informa&ccedil;&atilde;o.\"><br> \n" );
		fclose ( reading );
		
		printf ( "     </form>\n" );
		createButton ( "Voltar para a busca", "geSearchBeer.cgi", geLanguage, geNickname, ":<" );
	...
	else
	{
		...
		printf ( "     </form>\n" );
		createButton ( "Go back to the Search.", "geSearchBeer.cgi", geLanguage, geNickname, ":<" );
...
/* $RCSfile: geCgiExecuteSearchBeer.c,v $ */
