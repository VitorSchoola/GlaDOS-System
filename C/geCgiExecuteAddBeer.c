...
main ( int argc, char *argv[] )
{
	...
	char geBeer [BEER_NAME_MAX_LENGTH + 1];
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( userLoggedIn.userIdentifier == 4 )
	{
		if ( language == portuguese ) 
		{
			...
			printf ( "<div align=\"center\">" );
			createButton ( "Voltar para a p&aacute;gina anterior.", "geAddBeer.cgi", geLanguage, geNickname, "18" );
		...
		else
		{
			...
			printf ( "<div align=\"center\">" );
			createButton ( "Go back.", "geAddBeer.cgi", geLanguage, geNickname, "18" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "geBeer", geBeer, BEER_NAME_MAX_LENGTH ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
			mlCgiShowErrorPage ( "Error", "N&atilde;o conseguimos reaver o nome da nova cerveja.", ""  );
			printf ( "<div align=\"center\">" );
			createButton ( "Voltar para a p&aacute;gina anterior.", "geAddBeer.cgi", geLanguage, geNickname, "18" );
			printf ( "<br><a href=\"geLogout.cgi?geLanguage=portuguese&idString=%s\">Logout</a>", idString );
			printf ( "</div>" );
		...
		{
			mlCgiShowErrorPage ( "Error", "Could not retrieve the new beer's name.", ""  );
			printf ( "<div align=\"center\">" );
			createButton ( "Go back.", "geAddBeer.cgi", geLanguage, geNickname, "18" );
			printf ( "<br><a href=\"geLogout.cgi?geLanguage=english&idString=%s\">Logout</a>", idString );
			printf ( "</div>" );
	...
	if ( language == portuguese )
	{
		...
		printf("	Criando cerveja %s...<br>\n", geBeer );
		
		/* Check if Beers file already exists */
		
		if ( fileExists ( "../Files/", "Beers" ) == 0 )
		{
			writing = fopen ( "../Files/Beers", "w" );
			fclose( writing );
			firstLine = 1;
		}
		else
		...
		writing = fopen ( "../Files/Beers", "a" );
		
		if ( firstLine == 1 )
			fprintf ( writing, "b|%s", geBeer );
		else
			fprintf ( writing, "\nb|%s", geBeer );
		fprintf ( writing, "\nc|%s", geComment );
		fprintf ( writing, "\nr|000.00" );
			...
			if ( linha[0] == 'b' )
				if ( linha[1] == '|' )
				{
					if ( strcmp ( &linha[2], geBeer ) == 0 )
					{
						...
						createButton ( "Go back to the Previous Page.", "geAddBeer.cgi", geLanguage, geNickname, "18" );
...
/* $RCSfile: geCgiExecuteAddBeer.c,v $ */
