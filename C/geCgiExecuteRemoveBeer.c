 ...
 * $Log: geCgiExecuteRemoveBeer.c,v $
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		printf (" <b>Cerveja Deletada</b>: %s<br>", geBeerName );
		
		printf ( "     </form>\n" );
		createButton ( "Voltar para p&aacute;gina de sele&ccedil;&atilde;o de cerveja.", "geRemoveBeer.cgi", geLanguage, geNickname, "444" );
	...
	else
	{
		...
		printf (" <b>Deleted Beer</b>: %s<br>", geBeerName );
		
		printf ( "     </form>\n" );
		createButton ( "Go back to the Selection Page.", "geRemoveBeer.cgi", geLanguage, geNickname, "444" );
...
/* $RCSfile: geCgiExecuteRemoveBeer.c,v $ */
