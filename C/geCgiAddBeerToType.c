...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		if ( userLoggedIn.group == 1 )
		{
			...
			printf ("<form action=\"geExecuteAddBeerToType.cgi\" method=\"post\"> \n" );
			...
			printf (" <b>Listagem de todas as cervejas</b>:<br>\n" );
			
			/* Check if Beer File exists */
			
			if ( fileExists ( "../Files/", "Beers" ) == 0 )
			{
				printf ( "</form>" );
				mlCgiShowErrorPage ( "Ops", "N&atilde;o h&aacute; nenhuma cerveja no sistema.", "" );
				printf ( "<div align=\"center\">" );
			...
			while ( fgets ( linha, BEER_NAME_MAX_LENGTH + COMMENT_MAX_LENGTH + 8, reading ) != NULL )
			{
				...
				if ( linha[0] == 'b' )
					if ( linha[1] == '|' )
						printf ( "<input type='radio' name='geBeerName' value='%s'>%s<br>\n", &linha[2], &linha[2] );
			}
			fclose ( reading );
			
			printf (" <b>Listagem de todos tipos de cervejas</b>:<br>\n" );
			
			/* Check if Beer Types File exists */
			
			if ( fileExists ( "../Files/", "BeerTypes" ) == 0 )
			{
				printf ( "</form>" );
				mlCgiShowErrorPage ( "Ops", "N&atilde;o h&aacute; nenhum tipo de cerveja no sistema.", "" );
				printf ( "<div align=\"center\">" );
				createButton ( "Voltar ao Menu Principal", "geShowMenu.cgi", geLanguage, geNickname, "0" );
				printf ( "<br><a href=\"geLogout.cgi?geLanguage=portuguese&idString=%s\">Logout</a>", idString );
				printf ( "</div>" );
	...
	else
	{
		...
		if ( userLoggedIn.group == 1 )
		{
			printf("    Add Beer to Beer Type: <br>\n" );
			printf ("<form action=\"geExecuteAddBeerToType.cgi\" method=\"post\"> \n" );
			printf ("  <input type='hidden' name='geLanguage' value='%s'>\n", geLanguage );
			printf ("  <input type='hidden' name='nickname' value='%s'>\n", geNickname );
			...
			printf (" <b>Listing of all Beers</b>:<br>\n" );
			...
			{
				printf ( "</form>" );
				mlCgiShowErrorPage ( "Ops", "There is no beer in our system.", "" );
				printf ( "<div align=\"center\">" );
			...
			printf (" <b>Listing of all Beer Types</b>:<br>\n" );
			...
			{
				printf ( "</form>" );
				mlCgiShowErrorPage ( "Ops", "There is no Beer Type in our system.", "" );
				printf ( "<div align=\"center\">" );
				createButton ( "Voltar ao Menu Principal", "geShowMenu.cgi", geLanguage, geNickname, "0" );
				printf ( "<br><a href=\"geLogout.cgi?geLanguage=english&idString=%s\">Logout</a>", idString );
				printf ( "</div>" );
...
/* $RCSfile: geCgiAddBeerToType.c,v $ */
