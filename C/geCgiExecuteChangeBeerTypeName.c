 ...
 * $Log: geCgiExecuteChangeBeerTypeName.c,v $
...
main ( int argc, char *argv[] )
{
	...
	char geBeerType [BEER_NAME_MAX_LENGTH + 1], geBeerTypeNew[BEER_NAME_MAX_LENGTH + 1];
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "geBeerType", geBeerType, BEER_NAME_MAX_LENGTH ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
			...
			printf ( "<div align=\"center\">" );
			createButton ( "Voltar para a p&aacute;gina anterior.", "geChangeBeerTypeName.cgi", geLanguage, geNickname, "18" );
		...
		else
		{
			...
			printf ( "<div align=\"center\">" );
			createButton ( "Go back.", "geChangeBeerTypeName.cgi", geLanguage, geNickname, "18" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "geBeerTypeNew", geBeerTypeNew, BEER_NAME_MAX_LENGTH ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
			mlCgiShowErrorPage ( "Error", "N&atilde;o conseguimos reaver o novo nome do tipo cerveja.", ""  );
			printf ( "<div align=\"center\">" );
			createButton ( "Voltar para a p&aacute;gina anterior.", "geChangeBeerTypeName.cgi", geLanguage, geNickname, "18" );
			printf ( "<br><a href=\"geLogout.cgi?geLanguage=portuguese&idString=%s\">Logout</a>", idString );
			printf ( "</div>" );
	...
	if ( language == portuguese )
	{
		...
		printf("    Mudar nome de Tipo de Cervejas: <br>\n" );
		
		/* Checks if new beer type's name is already in use */
		...
		while ( fgets ( linha, BEER_NAME_MAX_LENGTH + COMMENT_MAX_LENGTH + 8, reading ) != NULL )
		{
			...
			if ( linha[0] == 't' )
				if ( linha[1] == '|' )
					if ( strcmp ( geBeerTypeNew, &linha[2] ) == 0 )
					{
		...
		reading = fopen ( "../Files/BeerTypes", "r" );
		writing = fopen ( "../Files/BeerTypes.c", "w" );
			...
			if ( linha[0] == 't' )
				if ( linha[1] == '|' )
				{
					if ( strcmp ( geBeerType, &linha[2] ) == 0 )
					{
						if ( firstLine == 1 )
						{
							fprintf ( writing, "t|%s" ,geBeerTypeNew );
							firstLine = 0;
						}
						else
							fprintf ( writing, "\nt|%s" ,geBeerTypeNew );					
		...
		fclose ( writing );
		fclose ( reading );
		remove ( "../Files/BeerTypes" );
		rename ( "../Files/BeerTypes.c", "../Files/BeerTypes" );
		...
		printf (" <b>Novo nome</b>: %s<br>", geBeerTypeNew );
		
		printf ( "     </form>\n" );
		createButton ( "Voltar para p&aacute;gina de sele&ccedil;&atilde;o de tipo de cerveja.", "geChangeBeerTypeName.cgi", geLanguage, geNickname, "444" );
	...
	else
	{
		...
		printf("    Change Beer Type's name: <br>\n" );
			...
			if ( linha[0] == 't' )
				if ( linha[1] == '|' )
					if ( strcmp ( geBeerTypeNew, &linha[2] ) == 0 )
					{
						...
						printf ( "<div align=\"center\">" );
						createButton ( "Go back to the Previous Page.", "geChangeBeerTypeName.cgi", geLanguage, geNickname, "18" );
						printf ( "<br><a href=\"geLogout.cgi?geLanguage=english&idString=%s\">Logout</a>", idString );
						printf ( "</div>" );
						fclose ( reading );
						mlCgiFreeResources ( );
		...
		printf (" <b>New Beer Type's name</b>: %s<br>", geBeerTypeNew );
		
		printf ( "     </form>\n" );
		createButton ( "Go back to the Selection Page.", "geChangeBeerTypeName.cgi", geLanguage, geNickname, "444" );
...
/* $RCSfile: geCgiExecuteChangeBeerTypeName.c,v $ */
