 ...
 * $Date: 2015/01/12 15:51:49 $
 * $Log: geCgiListUser.c,v $
 * Revision 1.2  2015/01/12 15:51:49  vitor.schoola
 * *** empty log message ***
 *
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
		...
		if ( userLoggedIn.group == 1 )
		{
			...
			while ( fgets ( linha, ID_LENGTH + NICKNAME_MAX_LENGTH + PASSWORD_MAX_LENGTH + GROUP_LENGTH + NAME_MAX_LENGTH + 6, reading) != NULL )
			{
				...
				for ( counter2 = counter + 1; counter2 <= ID_LENGTH + NICKNAME_MAX_LENGTH + 2; counter2++ )
					...
					printf ( "<b>Usu&aacute;rio:</b> %s", &linha[counter + 1] );
					if ( linha[counter2 + 1] == ';' )
						printf ( " <b>( Convite pendente )</b> " );
					printf ( "<br>\n" );
			}
		}
	...
	else
	{
		...
		if ( userLoggedIn.group == 1 )
		{
			...
			{
				printf ( "</form>" );
				mlCgiShowErrorPage ( "Ops", "O sistema n&atilde;o foi iniciado.", "" );
				printf ( "<div align=\"center\">" );
				createButton ( "Voltar ao Menu Principal", "geShowMenu.cgi", geLanguage, geNickname, "0" );
			...
			while ( fgets ( linha, ID_LENGTH + NICKNAME_MAX_LENGTH + PASSWORD_MAX_LENGTH + GROUP_LENGTH + NAME_MAX_LENGTH + 6, reading) != NULL )
			{
				...
				for ( counter2 = counter + 1; counter2 <= ID_LENGTH + NICKNAME_MAX_LENGTH + 2; counter2++ )
					...
					printf ( "<b>User:</b> %s", &linha[counter + 1] );
					if ( linha[counter2 + 1] == ';' )
						printf ( " <b>( Pending Invite )</b> " );
					printf ( "<br>\n" );
			}
		}
