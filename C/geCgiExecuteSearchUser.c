 ...
 * $Log: geCgiExecuteSearchUser.c,v $
...
main ( int argc, char *argv[] )
{
	...
	char geSearchUser [NICKNAME_MAX_LENGTH + 1];
	geUserDataType userLoggedIn;
	geUserDataType searchingUser;
	FILE *reading;
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "searchUserName", geSearchUser, NICKNAME_MAX_LENGTH ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
			mlCgiShowErrorPage ( "Error", "N&atilde;o conseguimos reaver palavra-chave desejada.", "" );
			printf ( "<div align=\"center\">" );
			createButton ( "Navegar para a p&aacute;gina anterior.", "geSearchUser.cgi", geLanguage, geNickname, "18" );
			printf ( "<a href=\"geLogout.cgi?geLanguage=portuguese&idString=%s\">Logout</a>", idString );
			printf ( "</div>" );
		}
		else
		{
			mlCgiShowErrorPage ( "Error", "Couldn't retrieve keyword.", ""  );
			printf ( "<div align=\"center\">" );
			createButton ( "Sail back to the previous page.", "geSearchUser.cgi", geLanguage, geNickname, "18" );
			printf ( "<a href=\"geLogout.cgi?geLanguage=english&idString=%s\">Logout</a>", idString );
			printf ( "</div>" );
		}
		mlCgiFreeResources ( );
	...
	if ( language == portuguese )
	{
		...
		printf("    Procurar Usu&aacute;rio: <br>\n" );
		printf ("<form action=\"geChosenSearchUser.cgi\" method=\"post\"> \n" );
		...
		while ( fgets ( linha, ID_LENGTH + NICKNAME_MAX_LENGTH + PASSWORD_MAX_LENGTH + GROUP_LENGTH + NAME_MAX_LENGTH + 6, reading) != NULL )
		{
			...
			CheckUserInfo ( &linha[counter + 1], &searchingUser );
			if ( ( strstr ( searchingUser.email , geSearchUser ) != NULL ) || ( strstr ( searchingUser.name , geSearchUser ) != NULL ) )
				printf ( "<input type='radio' name='searchUserName' value='%s'>%s ( %s )<br>\n", searchingUser.email, searchingUser.email, searchingUser.name );
		}		
		printf ("  <input type=\"submit\" name=\"submit\" value=\"Ver informa&ccedil;&atilde;o.\"><br> \n" );
		
		fclose ( reading );
		
		printf ( "     </form>\n" );
		createButton ( "Voltar para a busca", "geSearchUser.cgi", geLanguage, geNickname, "s2" );
	...
	else
	{
		...
		printf ("  <input type=\"submit\" name=\"submit\" value=\"Check Informations.\"><br> \n" );
		
		fclose ( reading );
		
		printf ( "     </form>\n" );
		createButton ( "Go back to the Search", "geSearchUser.cgi", geLanguage, geNickname, "s2" );
...
/* $RCSfile: geCgiExecuteSearchUser.c,v $ */
