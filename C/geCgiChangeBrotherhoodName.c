 ...
 * $Date: 2015/01/10 19:08:54 $
 * $Log: geCgiChangeBrotherhoodName.c,v $
 * Revision 1.1  2015/01/10 19:08:54  vitor.schoola
 * Initial revision
 *
 *
 */
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		printf("    Editar Nome de Confrarias: <br>\n" );
		printf ("<form action=\"geChosenEditBrotherhood.cgi\" method=\"post\"> \n" );
		...
		{
			printf (" Listagem de todas confrarias:<br> \n" );
			
			/* Check if Brotherhood File exists */
			...
			while ( fgets ( linha, NICKNAME_MAX_LENGTH + 2, reading) != NULL )
			{
				...
				if ( linha[0] == 'b' )
					if ( linha[1] == '|' ) 
						printf ( "<input type='radio' name='editBrotherhoodName' value='%s'><b>%s</b><br>\n", &linha[2], &linha[2] );
			}
	...
	else
	{
		...
		printf("    Edit Brotherhood's Name: <br>\n" );
		printf ("<form action=\"geChosenEditBrotherhood.cgi\" method=\"post\"> \n" );
		printf ("  <input type='hidden' name='geLanguage' value='%s'>\n", geLanguage );
		printf ("  <input type='hidden' name='nickname' value='%s'>\n", geNickname );
		...
		if ( userLoggedIn.group == 1 )
		{
			printf (" Listing of all Brotherhoods:<br> \n" );
...
/* $RCSfile: geCgiChangeBrotherhoodName.c,v $ */
