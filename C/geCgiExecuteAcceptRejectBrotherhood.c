 ...
 * $Date: 2015/01/10 19:10:34 $
 * $Log: geCgiExecuteAcceptRejectBrotherhood.c,v $
 * Revision 1.1  2015/01/10 19:10:34  vitor.schoola
 * Initial revision
 *
 *
 */
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "brotherhoodName", geBrotherhoodName, BROTHERHOOD_MAX_LENGTH ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
			...
			printf ( "<div align=\"center\">" );
			createButton ( "Voltar para a p&aacute;gina anterior.", "geAcceptRejectBrotherhood.cgi", geLanguage, geNickname, "18" );
		...
		else
		{
			...
			printf ( "<div align=\"center\">" );
			createButton ( "Go back.", "geAcceptRejectBrotherhood.cgi", geLanguage, geNickname, "18" );
	...
	if ( mlCgiGetFormStringNoNewLines ( "accorrej", selectedOpt, 4 ) != ML_CGI_OK )
	{
		if ( language == portuguese ) 
		{
	...
	if ( language == portuguese )
	{
		...
		/* Removes user from the invited list 
		    It will happen either if the user accepted or rejected the invite.*/
		
		reading = fopen ( "../Files/Brotherhood.invited", "r" );
		writing = fopen ( "../Files/Brotherhood.invited.r", "w" );
		...
		rename ( "../Files/Brotherhood.invited.r", "../Files/Brotherhood.invited" );
		
		/* Appends new user to brotherhood if accepted or do not do shit if rejected */

		firstLine = 1;
		writtenNewName = 0;
		
		if ( selectedOpt[0] == 'a' ) // [a]ccepted
		{
			writing = fopen ( "../Files/Brotherhoods.aiuser", "w" );
			...
			rename ( "../Files/Brotherhoods.aiuser", "../Files/Brotherhoods" );
			
		}
		else // [r]ejected
		{
			// Simply does not add him in the user list of selected brotherhood
		}
	...
	else
	{
		...
		createButton ( "Go back to the Previous Page.", "geAcceptRejectBrotherhood.cgi", geLanguage, geNickname, "18" );
...
/* $RCSfile: geCgiExecuteAcceptRejectBrotherhood.c,v $ */
