 ...
 * $Log: geCgiRemoveBrotherhood.c,v $
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		printf("    Remover Confrarias: <br>\n" );
		printf ("<form action=\"geExecuteRemoveBrotherhood.cgi\" method=\"post\"> \n" );
		...
		if ( userLoggedIn.group == 1 )
		{
			...
			while ( fgets ( linha, NICKNAME_MAX_LENGTH + 2, reading) != NULL )
			{
				...
				if ( linha[0] == 'b' )
					if ( linha[1] == '|' ) 
						printf ( "<input type='radio' name='removeBrotherhoodName' value='%s'><b>%s</b><br>\n", &linha[2], &linha[2] );
			}
...
/* $RCSfile: geCgiRemoveBrotherhood.c,v $ */
