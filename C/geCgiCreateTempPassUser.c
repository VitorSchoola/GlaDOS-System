 ...
 * $Date: 2015/01/10 19:09:59 $
 * $Log: geCgiCreateTempPassUser.c,v $
 * Revision 1.1  2015/01/10 19:09:59  vitor.schoola
 * Initial revision
 *
 *
 */
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		printf("    Criar senha tempor&aacute;ria para Usu&aacute;rios: <br>\n" );
		printf ("<form action=\"geExecuteCreateTempPassUser.cgi\" method=\"post\"> \n" );
		...
		if ( userLoggedIn.group == 1 )
		{
			...
			while ( fgets ( linha, ID_LENGTH + NICKNAME_MAX_LENGTH + PASSWORD_MAX_LENGTH + GROUP_LENGTH + NAME_MAX_LENGTH + 6, reading) != NULL )
			{
				...
				for ( counter2 = counter + 1; counter2 <= ID_LENGTH + NICKNAME_MAX_LENGTH + 2; counter2++ )
					...
					if ( linha[counter2 + 1] != ';' )
						printf ( "<input type='radio' name='editUserName' value='%s'><b>%s</b><br>\n", &linha[counter + 1], &linha[counter + 1] );
			}
			printf ("  <input type=\"submit\" name=\"submit\" value=\"Criar senha\"><br> \n" );
		}
	...
	else
	{
		...
		printf("    Create a Temporary Password for User: <br>\n" );
		printf ("<form action=\"geExecuteCreateTempPassUser.cgi\" method=\"post\"> \n" );
		printf ("  <input type='hidden' name='geLanguage' value='%s'>\n", geLanguage );
		printf ("  <input type='hidden' name='nickname' value='%s'>\n", geNickname );
		...
		if ( userLoggedIn.group == 1 )
		{
			...
			printf ("  <input type=\"submit\" name=\"submit\" value=\"Create Password\"><br> \n" );
		}
...
/* $RCSfile: geCgiCreateTempPassUser.c,v $ */
