 ...
 * $Date: 2015/01/10 19:09:44 $
 * $Log: geCgiChosenSearchUser.c,v $
 * Revision 1.1  2015/01/10 19:09:44  vitor.schoola
 * Initial revision
 *
 *
 */
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	CheckUserInfo ( geSearchUser, &searchingUser );
	...
	if ( language == portuguese )
	{
		...
		printf ( " Email: %s <br> Senha: ------- <br> Grupo: %c <br> Nome: %s <br>\n", searchingUser.email, searchingUser.group + '0',searchingUser.name );
	...
	else
	{
		...
		printf ( " Email: %s <br> Password: ------- <br> Group: %c <br> Real Name: %s <br>\n", searchingUser.email, searchingUser.group + '0',searchingUser.name );
	
		createButton ( "Go back to the Search Page", "geSearchUser.cgi", geLanguage, geNickname, "s2" );
...
/* $RCSfile: geCgiChosenSearchUser.c,v $ */
