 ...
 * $Log: geCgiExecuteAddBeerToType.c,v $
...
main ( int argc, char *argv[] )
{
	...
	char geBeerType [BEER_NAME_MAX_LENGTH + 1], geBeerName [BEER_NAME_MAX_LENGTH + 1];
	char linha [ BEER_NAME_MAX_LENGTH + COMMENT_MAX_LENGTH + 8 + 2 ];
	FILE *reading, *writing;
	unsigned firstLine = 1;
	unsigned selectedBeerType = 0;
...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		printf("    Adicionar Cerveja ao Tipo de Cervejas: <br>\n" );
		
		/* Checks if beer's name is already in selected type */
		...
		while ( fgets ( linha, BEER_NAME_MAX_LENGTH + COMMENT_MAX_LENGTH + 8, reading ) != NULL )
		{
			...
			if ( selectedBeerType == 1 )
				if ( linha[0] == 'u' )
					if ( linha[1] == '|' )
						if ( strcmp ( &linha[2], geBeerName ) == 0 )
						{
							printf ( "</form>" );
							mlCgiShowErrorPage ( "Ops", "Tipo de cerveja j&aacute; cont&eacute;m cerveja.", "" );
							printf ( "<div align=\"center\">" );
							createButton ( "Voltar ao Menu Principal", "geShowMenu.cgi", geLanguage, geNickname, "0" );
							createButton ( "Voltar para a p&aacute;gina anterior", "geAddBeerToType.cgi", geLanguage, geNickname, "1" );
							printf ( "<br><a href=\"geLogout.cgi?geLanguage=portuguese&idString=%s\">Logout</a>", idString );
							printf ( "</div>" );
							mlCgiFreeResources ( );
							exit ( ML_CGI_OK );
		...
		/* Writes new beer's name */
		
		reading = fopen ( "../Files/BeerTypes", "r" );
		writing = fopen ( "../Files/BeerTypes.ab", "w" );
		...
		rename ( "../Files/BeerTypes.ab", "../Files/BeerTypes" );
		...
		printf ( "     </form>\n" );
		createButton ( "Voltar para p&aacute;gina de sele&ccedil;&atilde;o.", "geAddBeerToType.cgi", geLanguage, geNickname, "444" );
		createButton ( "Voltar ao Menu Principal", "geShowMenu.cgi", geLanguage, geNickname, "0" );
	...
	else
	{
		...
		printf("    Add Beer in a selected Beer Type: <br>\n" );
			...
			if ( selectedBeerType == 1 )
				if ( linha[0] == 'u' )
					if ( linha[1] == '|' )
						...
						{
							printf ( "</form>" );
							mlCgiShowErrorPage ( "Ops", "Beer is already in Beer Type group.", "" );
							printf ( "<div align=\"center\">" );
							createButton ( "Go back to the Main Menu", "geShowMenu.cgi", geLanguage, geNickname, "0" );
							createButton ( "Go back to the Previous Page", "geAddBeerToType.cgi", geLanguage, geNickname, "1" );
							printf ( "<br><a href=\"geLogout.cgi?geLanguage=english&idString=%s\">Logout</a>", idString );
							printf ( "</div>" );
							mlCgiFreeResources ( );
							exit ( ML_CGI_OK );
		...
		printf ( "     </form>\n" );
		createButton ( "Go back to the Selection Page.", "geAddBeerToType.cgi", geLanguage, geNickname, "444" );
		createButton ( "Go back to the Main Menu", "geShowMenu.cgi", geLanguage, geNickname, "0" );
...
/* $RCSfile: geCgiExecuteAddBeerToType.c,v $ */
