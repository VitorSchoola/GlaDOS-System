...
//	printf("        alert ( \"Cookie Found\" ); \n" );
	...
	if ( language == portuguese )
	{
		...
		printf(" Listagem de todos tipos de cervejas:<br> \n" );
		...
		while ( fgets ( linha, BEER_NAME_MAX_LENGTH + COMMENT_MAX_LENGTH + 8, reading ) != NULL )
		{
			...
			if ( linha[0] == 'c' )
				if ( linha[1] == '|' )
				{
					printf ( "<b>Coment&aacute;rio</b>:<br> %s<br><br>\n", &linha[2] );
				}
			if ( linha[0] == 'd' )
				if ( linha[1] == '|' )
				{
	...
	else
	{
		...
		printf(" Listing of all Beer Types:<br> \n" );
